...
async def checkout_purchase(
    ...
    :return:
    """
    logger.info(f"Создать заказ на основе текущей корзины пользователя {current_user.id}")
...
async def list_purchases(
    ...
    result = await PurchaseService.get_list_purchases(uow, current_user, page, page_size)
    return result
...
async def get_purchase(
    ...
    """
    logger.info(f"Получить детальную информацию по заказу пользователя {current_user.id}.")
